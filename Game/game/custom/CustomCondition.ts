...
module CustomCondition {
    ...
    export function f1(trigger: CommandTrigger, p: CustomConditionParams_1): boolean {
        ...
        if (p.type == 20) return (so.getModule(6) as SoModule_Battler).mustInBattle;
        if (p.type == 21) return !GameBattleHelper.isPlayerControlEnabledBattler(so);
        if (p.type == 22) {
            let skillID = p.skillUseVar ? Game.player.variable.getVariable(p.skillIDVarID) : p.skillID;
            return Game.getActorSkillBySkillID((so.getModule(6) as SoModule_Battler).actor, skillID) != null;
        }
        if (p.type == 23) {
            let equipID = p.equipUseVar ? Game.player.variable.getVariable(p.equipIDVarID) : p.equipID;
            return Game.getActorEquipByEquipID((so.getModule(6) as SoModule_Battler).actor, equipID) != null;
        }
        if (p.type == 24) {
            let itemID = p.itemUseVar ? Game.player.variable.getVariable(p.itemIDVarID) : p.itemID;
            return Game.getActorItemByItemID((so.getModule(6) as SoModule_Battler).actor, itemID) != null;
        }
        if (p.type == 25) {
            let statusID = p.statusUseVar ? Game.player.variable.getVariable(p.statusIDVarID) : p.statusID;
            let status = GameBattleHelper.getBattlerStatus(so, statusID);
            return status && status.currentLayer >= p.statusLayer;
        }
        if (p.type == 26) {
            let classID = p.classUseVar ? Game.player.variable.getVariable(p.classIDVarID) : p.classID;
            return (so.getModule(6) as SoModule_Battler).actor.class == classID;
        }
        if (p.type == 27) {
            return (so.getModule(6) as SoModule_Battler).isDead;
        }
        if (p.type == 28) {
            return (so.getModule(6) as SoModule_Battler).moved;
        }
        if (p.type == 29) {
            return (so.getModule(6) as SoModule_Battler).actioned;
        }
        if (p.type == 30) {
        ...
        if (p.type == 33) {
            return (so.getModule(6) as SoModule_Battler).hateList.length != 0;
        }
    }
    /**
     * 界面
    ...
    export function f9(trigger: CommandTrigger, cp: CustomConditionParams_9): boolean {
        // -- 获取角色
        let actorDS = ProjectGame.getPlayerActorByCheckType(cp.actorCheckType, cp.actorUseVar, cp.actorID, cp.actorIDVarID,
