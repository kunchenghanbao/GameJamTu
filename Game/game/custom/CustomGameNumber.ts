...
module CustomGameNumber {
    ...
    export function f2(trigger: CommandTrigger, p: CustomGameNumberParams_2): number {
        ...
        if (p.type == 15) {
            ...
            return getActorAttributes(actor, p.actorAttrName, p.actorCustomAttr.varName, p.trend, p.extendAttribute);
        }
    }
    /**
     * 场景对象关系
    ...
    export function f4(trigger: CommandTrigger, p: CustomGameNumberParams_4): number {
        ...
        if (p.type == 3) {
            let actorID = p.actorUseVar ? Game.player.variable.getVariable(p.actorIDVarID) : p.actorID;
        ...
        if (p.type == 4) {
            ...
            if (p.actorCheckType == 0) {
                let actorID = p.actorUseVar ? Game.player.variable.getVariable(p.actorIDVarID) : p.actorID;
                actorDS = ProjectPlayer.getPlayerActorDSByActorID(actorID);
            }
            else {
                let inPartyIndex = p.actorInPartyIndexVarIDUseVar ? Game.player.variable.getVariable(p.actorInPartyIndexVarID) : p.actorInPartyIndex;
                actorDS = ProjectPlayer.getPlayerActorDSByInPartyIndex(inPartyIndex);
            }
            if (!actorDS) return 0;
            return getActorAttributes(actorDS.actor, p.actorAttrName, p.actorCustomAttr.varName, p.trend, p.extendAttribute);
        }
        if (p.type == 5) {
    ...
    export function f6(trigger: CommandTrigger, p: CustomGameNumberParams_6): number {
        if (p.type == 0) return stage.mouseX;
            ...
            for (let i = 0; i < GUI_Setting.SYSTEM_KEYS.length; i++) {
    ...
    export function f11(trigger: CommandTrigger, p: CustomGameNumberParams_11): number {
        if (p.inBattleType == 0) {
            switch (p.type1) {
                ...
                case 1:
                    let actorDS = ProjectPlayer.getPlayerActorDSByInPartyIndex(GUI_Package.actorSelectedIndex)
            ...
            switch (p.type2) {
                ...
                case 3:
                    if (GameBattleAction.fromBattlerSkillGridPos) {
                        let sceneX = GameBattleAction.fromBattlerSkillGridPos.x * Config.SCENE_GRID_SIZE + Config.SCENE_GRID_SIZE * 0.5;
                        let sceneY = GameBattleAction.fromBattlerSkillGridPos.y * Config.SCENE_GRID_SIZE + Config.SCENE_GRID_SIZE * 0.5;
                        let globalP = Game.currentScene.getGlobalPos(sceneX, sceneY);
                        return globalP.x;
                    }
                    return -1;
                case 4:
                ...
                case 6:
                    return GameBattleHelper.overCursorNoDeadBattler ? GameBattleHelper.overCursorNoDeadBattler.index : -1;
            }
        }
        return 0;
    }
    ...
    // 
    //------------------------------------------------------------------------------------------------------
    export let attributeRes: { MaxHP: number; MaxSP: number; ATK: number; DEF: number; MAG: number; MagDef: number; HIT: number; DOD: number; CRIT: number; MagCrit: number; MoveGrid: number; statusAddMaxHP: number; extendAttributes: number[] };
    function getActorAttributes(actor: Module_Actor, attributeIndex: number, actorCustomAttrName: string, trend: boolean, extendAttribute: number): number {
        switch (attributeIndex) {
            ...
            case 2:
                let lv = GameBattleHelper.getLevelByActor(actor);
            ...
            case 15:
                ...
                return battler ? battler.index : -1;
            case 16:
                return actor.atkMode;
            case 17:
                return actor.hitFrame;
            case 18:
                return trend && attributeRes ? attributeRes.MoveGrid : actor.MoveGrid;
            case 19:
                let attrValue = actor[actorCustomAttrName];
