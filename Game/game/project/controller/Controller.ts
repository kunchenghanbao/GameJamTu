...
class Controller {
    ...
    static get inSceneEnabled() {
        ...
        // 战斗中且打开了相关界面时
        if (GameBattleHelper.isInBattle && (GameDialog.isInDialog || GameUI.isOpened(16) || GameUI.isOpened(22) || GameUI.isOpened(23) || GameUI.isOpened(25) || GameUI.isOpened(26) || GameUI.isOpened(27))) {
            return false;
        }
    ...
    static startJoy(dirAngle: number, recordAngle: boolean = true, tryTimes: number = -1, oriAngle: number = null): void {
        ...
        TouchCameraControl.followPlayerOnMove();
        // 中心点模式
     ...
     * @param force [可选] 默认值=false 强制（允许在战斗中调用）
     * @param testMode 
     * @return [boolean] 是否成功
     */
    static startSceneObjectTouchEvent(trigger: ProjectClientSceneObject, executor: ProjectClientSceneObject, onCommandExecuteOver: Callback = null, force: boolean = false, testMode: boolean = false, delayFrame: number = null): boolean {
        // 如果还未真正登陆场景则忽略（即预先摆放接触的对象不会触发碰触事件）
        if (!force && GameGate.gateState < GameGate.STATE_4_PLAYER_CONTROL_START) {
            Callback.New(Controller.startSceneObjectTouchEvent, Controller, [trigger, executor, onCommandExecuteOver]).delayRun(0);
            return false;
        }
        // 战斗时忽略
        if (!force && GameBattleHelper.isInBattle) return;
        ...
        // 不允许非玩家以外的对象执行时则忽略
        if (executor.onlyPlayerTouch) {
            if (GameBattleHelper.isInBattle) {
                if (!GameBattleHelper.isPlayerCamp(trigger)) return false;
            }
            else {
                if (trigger != Game.player.sceneObject) return false;
            }
        }
        // 如果需要玩家等待的场合
        if (executor.waitTouchEvent && trigger == Game.player.sceneObject) {
            ...
            // 调用「碰触事件」前的处理
            if (!testMode) GameCommand.startCommonCommand(14013, [], null, trigger, executor);
            ...
            let touchCompleteCallback = Callback.New(() => {
            ...
            let f = () => {
                let touchEventSuccess = GameCommand.startSceneObjectCommand(executor.index, 1, null, touchCompleteCallback, trigger);
                if (!touchEventSuccess) touchCompleteCallback.run();
                return touchEventSuccess;
            }
            if (delayFrame != null) {
                setFrameout(() => { f.apply(this); }, delayFrame);
                return true;
            }
            else {
                return f.apply(this);
            ...
            if (testMode) return true;
            if (delayFrame != null) {
                setFrameout(() => { GameCommand.startSceneObjectCommand(executor.index, 1, null, onCommandExecuteOver, trigger); }, delayFrame);
                return true;
            }
            else {
