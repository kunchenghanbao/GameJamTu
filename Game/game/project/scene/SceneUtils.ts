...
class SceneUtils {
     ...
     * @param except 排除者
     * @param checker 检查者
     * @param ignoreFriendlyBattlers 是否忽略与检查者同阵营的战斗者
     * @return [boolean] 
     */
    isObstacleGrid(gridP: Point, except: ProjectClientSceneObject = null, checker: ProjectClientSceneObject = null, ignoreFriendlyBattlers: boolean = false): boolean {
        if (this.isOutsideByGrid(gridP)) { return true; }
        // 检查是否具备动态穿透属性
        let gridStatus = this.getGridDynamicObsStatus(gridP, except, null, checker, ignoreFriendlyBattlers);
     ...
     * @param ignoreFriendlyBattlers [可选] 检查战斗移动时忽略与检查者同阵营的战斗者
     * @return [number] 0-无障碍 1-桥属性 2-存在障碍
     */
    getGridDynamicObsStatus(gridP: Point, excepter: ProjectClientSceneObject = null, gridSceneObjects: ProjectClientSceneObject[] = null, checker: ProjectClientSceneObject = null, ignoreFriendlyBattlers: boolean = false): number {
        // 忽略超过边界的情况
        ...
        for (let i = 0; i < len; i++) {
            ...
            // 战斗移动/强制位移可按调用方选择忽略同阵营战斗者；
            // 未开启该选项时，所有非穿透战斗者都会形成阻挡。
            // 桥属性仍保留原有的优先通行语义。
            if (ignoreFriendlyBattlers && checker && so !== checker && so.getModule && checker.getModule) {
                let soBattlerModule = so.getModule(6) as SoModule_Battler;
                let checkerBattlerModule = checker.getModule(6) as SoModule_Battler;
                if (soBattlerModule && checkerBattlerModule &&
                    soBattlerModule.battleCamp === checkerBattlerModule.battleCamp && !so.bridge) continue;
            }
