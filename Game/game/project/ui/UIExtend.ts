...
EventUtils.addEventListener(GameUI, GameUI.EVENT_CREATE_UI, Callback.New((ui: GUI_BASE) => {
    ...
    // 战斗者菜单的装备按钮由代码接入，避免改动原有通用指令数据。
    // 按钮只在玩家回合且没有进行中的指示器时允许打开装备界面。
    if ((ui as any).guiID == 22) {
        let battleMenu = ui as GUI_22;
        let equipButton = battleMenu.装备按钮;
        // 兼容已经缓存旧版 ui22.json 的客户端：资源未更新时动态补建按钮。
        if (!equipButton) {
            equipButton = new UIButton();
            equipButton.name = "装备按钮";
            equipButton.x = 1129;
            equipButton.y = 690;
            equipButton.width = 178;
            equipButton.height = 70;
            equipButton.label = "装备";
            equipButton.image1 = "asset/image/picture/control/btn_instruct_normal.png";
            equipButton.image2 = "asset/image/picture/control/btn_instruct_over.png";
            equipButton.image3 = "asset/image/picture/control/btn_instruct_click.png";
            equipButton.fontSize = 24;
            equipButton.color = "#fffae5";
            equipButton.overColor = "#fcf400";
            equipButton.clickColor = "#fcf400";
            equipButton.bold = true;
            equipButton.smooth = true;
            equipButton.align = 1;
            equipButton.valign = 1;
            equipButton.font = "Source_Han_Sans";
            equipButton.mouseEnabled = true;
            battleMenu.addChild(equipButton);
            battleMenu.装备按钮 = equipButton;
        }
        // 新版资源通过 server/ui/sui22.json 绑定事件；只有旧资源动态补建按钮时才直接监听。
        let hasConfiguredCommand = (equipButton as any).hasCommand && (equipButton as any).hasCommand[0];
        if (equipButton && !hasConfiguredCommand && !(equipButton as any).__battleEquipmentBound) {
            (equipButton as any).__battleEquipmentBound = true;
            equipButton.on(EventObject.CLICK, equipButton, () => {
                GameBattleController.openBattleEquipmentMenu();
            });
        }
    }
}, this));
