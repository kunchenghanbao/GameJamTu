...
class GUI_VirtualKeyboard extends GUI_12 {
     ...
     * 移动端敌方回合加速按钮
     */
    battleSpeed: MobileBattleSpeedControl;
    /**
     * 摇杆中心点
     */
    ...
    dispose(): void {
        // 小地图由场景级全局显示层持有，不随虚拟键盘销毁。
        this.miniMap = null;
        if (this.battleSpeed) {
            this.battleSpeed.dispose();
            this.battleSpeed = null;
        }
        if (GUI_VirtualKeyboard.self == this) GUI_VirtualKeyboard.self = null;
        super.dispose();
    }
    /**
     * 初始化
     */
    private init(): void {
        this.miniMap = MiniMapControl.ensureDesktop();
        this.battleSpeed = new MobileBattleSpeedControl(this);
        // “隐藏按键”仍由原有的 UI 自定义事件控制容器显隐。
        // 摇杆和方向键也属于同一容器，监听容器显隐只用于同步自身状态，
        // 不接管按钮点击，避免和原有的 A/B/START/BACK 统一隐藏逻辑重复触发。
        if (this.容器) {
            this.容器.on(UIBase.ON_VISIBLE_CHANGE, this, this.syncVirtualKeyboardVisibility);
            this.syncVirtualKeyboardVisibility();
        }
    ...
    /**
     * 原有的“隐藏按键”事件切换的是“容器”。方向键和摇杆也在该容器内，
     * 这里同步它们的自身 visible 状态，确保触摸命中检测在隐藏时不会拦截场景操作。
     */
    private syncVirtualKeyboardVisibility(): void {
        let visible = !this.容器 || this.容器.visible;
        if (!visible && this.touchId != null && this.rocker && this.rockerBg) {
            this.stopDragRocker(null);
        }
        if (this.rockerBg) this.rockerBg.visible = visible;
        if (this.dirBtnRoot) this.dirBtnRoot.visible = visible;
    }
    /**
     * 开始拖拽摇杆
     * @param e 
