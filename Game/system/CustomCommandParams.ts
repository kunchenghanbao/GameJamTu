...
class CustomCommandParams_9001 {
    ...
    preloadAssets: boolean; // = true; 预载入资源
    isShowLoadingUI: boolean; // = false; 显示加载界面
    bindingUI: { uiID: number, compName:string, compID:string, varName:string }; // 加载组件
    ...
    battleStageX_onLoadFile: string; // = ""; 战斗阶段：读档时
}
/**
* 自定义指令 9002-结束战斗
*/
class CustomCommandParams_9002 {
    ...
    disposePreloadAssets: boolean; // = true; 卸载此前预载入的战斗资源
}
/**
* 自定义指令 9003-战斗参数设定
*/
...
class CustomCommandParams_9009 {
    battleScene: number; // = 0; 战斗背景
    battleSceneBGM: string; // = ""; 战斗背景音乐
}
/**
* 自定义指令 9010-
*/
...
class CustomCommandParams_10008 {
    ...
    soUseVar: number; // = 0; 使用变量指定
    symbol: number; // = 0; 操作
    fromPlayerPackage: boolean; // = true; 从玩家背包中携带
    itemID: number; // = 1; 道具
    itemIDVarID: number; // = 1; 装备
    itemUseVar: number; // = 0; 使用变量
    takeOffItemSaveToVar: number; // = 1; 储存至
    isTakeOffItemSaveToVar: boolean; // = false; 卸下的道具编号储存至数值变量
}
/**
* 自定义指令 10009-修改角色的属性
*/
